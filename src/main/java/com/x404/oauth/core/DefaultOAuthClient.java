...
import com.x404.oauth.OAuthException;
import com.x404.oauth.config.OAuthConfig;
import com.x404.oauth.context.OAuthContext;
import com.x404.oauth.oauth2.protocol.AdditionalParamsAuthorizationGrant;
import com.x404.oauth.oidc.protocol.IdToken;
import com.x404.oauth.oidc.protocol.UserInfoTokenResponse;
...
import static com.x404.oauth.utils.UrlUtils.toURI;
...
public class DefaultOAuthClient implements OAuthClient
{
    ...
    public DefaultOAuthClient(OAuthConfig authConfig) {
        ...
        if( supportOIDC ) {
            ...
            try {
                JWKSource<SecurityContext> jwkSource = JWKSourceBuilder.create(jwkSetURI.toURL()).cache(true).build();
